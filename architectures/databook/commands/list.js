...
export async function runList(opts) {
  ...
  // ── Table output ───────────────────────────────────────────────────────────
  // id column: dynamic width up to MAX_ID chars.
  // IRIs longer than MAX_ID are truncated with '…'; --verbose shows the full IRI.
  const MAX_ID = 60;
  const idWidth = Math.min(
    Math.max(...rows.map(r => r.id.length), 4),
    MAX_ID
  );
  ...
  for (const row of rows) {
    const line = cols.map(c => {
      ...
      // Truncate IDs longer than the column width
      const val = raw.length > c.width ? raw.slice(0, c.width - 1) + '…' : raw;
      return val.padEnd(c.width);
    }).join('  ');
    process.stdout.write(line + '\n');

    // --verbose: show full IRI when it was truncated in the table
