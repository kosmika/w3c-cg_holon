 ...
 *   databook describe ... | databook convert --to json-ld   ← DataBook auto-detected
 *
...
export async function runConvert(inputArg, opts) {
  ...
  if (!filePath || filePath === '-') {
    ...
    if (fromFmt) {
      // Explicit format: pass through as-is (existing behaviour)
      inputContent = content;
      inputLabel   = fromFmt;
    } else {
      // No --from: attempt DataBook auto-detection before erroring
      const db = parseDataBook(content, null);

      if (db && db.blocks.some(b => b.id)) {
        // ── DataBook on stdin — same block selection as file mode ───────────
        if (listMode) {
        ...
        if (!bid) {
          ...
          if (named.length === 1) {
            bid = named[0].id;
            if (!quiet) process.stderr.write(`info: single named block — converting '${bid}'\n`);
          } else {
            const ids = named.map(b => b.id).join(', ');
            die(`--block-id required for DataBook stdin (available: ${ids})\n  Or use --from <format> to treat stdin as raw content`);
          }
        }
        ...
        if (!block) {
          ...
          die(`no block with id '${bid}' in DataBook stdin\n  Available: ${ids || 'none'}`);
        }

        inputContent = blockPayload(block);
        inputLabel   = block.label;

      } else {
        // Not a DataBook and no --from
        die('--from <format> is required when reading raw content from stdin (e.g. --from turtle)\n  If piping a DataBook, omit --from and the block will be auto-detected');
      }
    }
